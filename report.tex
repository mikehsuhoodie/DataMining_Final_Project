...
\and
\IEEEauthorblockN{314551096 CHI WEI LING}
\and
\IEEEauthorblockN{313554066 WU CHI CHUN}
}
...
\subsection{Multi-Fold Time-Based Local Validation}
\label{sec:multi_fold}

A single time-based validation split only checks one cutoff per region, so it
cannot reveal whether the local error estimate is sensitive to \emph{which}
historical period is held out. To address the multi-cutoff validation gap
noted above, and to make each local validation example mirror the Kaggle test
format as closely as possible (one 91-day input window producing five
horizon-specific predictions per region), we construct $K=3$ non-overlapping
validation folds per region.

For region $r$, let $t_1 < t_2 < \cdots < t_{m_r}$ denote the day indices that
carry a non-null weekly score, ordered from oldest to most recent. With
$w=5$ labeled weeks per fold, fold $k \in \{0,1,\ldots,K-1\}$ (where $k=0$ is
the most recent fold and is closest in time to the Kaggle test split) is
anchored at the cutoff
\begin{equation}
T_r^{(k)} = t_{\,m_r-(k+1)w+1} - 7.
\end{equation}
Fold $k$ contributes a single 91-day input window $X_{r,T_r^{(k)}}$, which is
reused to generate one validation example per horizon:
\begin{equation}
\Big(\phi\big(X_{r,T_r^{(k)}}, r, T_r^{(k)}\big),\;
      y_{r,\,T_r^{(k)}+7h}\Big), \quad h=1,\ldots,5.
\end{equation}
In other words, each fold behaves exactly like one test-time region: a fixed
91-day weather window that is scored against five future weekly labels, one
per horizon.
...
\begin{tabular}{cccc}
training cutoffs & fold 2 & fold 1 & fold 0 \\
cutoff $< T_2 - 7g$ &
$[T_2, T_2{+}35]$ &
$[T_1, T_1{+}35]$ &
$[T_0, T_0{+}35]$ \\
\end{tabular}}\\[4pt]
\scriptsize oldest $\longrightarrow$ newest \quad (fold 0 closest to the Kaggle test split)
\end{minipage}}
\caption{Layout of the $K=3$ non-overlapping validation folds. Here $T_k
\equiv T_r^{(k)}$ for brevity. Each fold $k$ reuses one 91-day input window
ending at $T_k$ to predict all five horizons $T_k{+}7,\ldots,T_k{+}35$.
Training cutoffs are restricted to targets earlier than fold 2's window,
optionally shifted further back by a gap of $g$ weeks.}
\label{fig:multi_fold_layout}
\end{figure}

Training examples for every horizon are drawn only from cutoffs whose
horizon-$h$ target lies strictly before fold 2's window, i.e.\ before
$t_{\,m_r-Kw+1}$, so that no validation label can leak into training. We also
introduce an optional gap of $g$ weeks (\texttt{train\_gap\_weeks}) that
removes the $g$ weeks immediately preceding fold 2 from the training pool,
emulating the roughly 60-week separation between Kaggle's visible training
history and its hidden test targets. All other settings (the 641-feature
filtered weather-only feature set, stride 8, a cap of 150{,}000 training
examples per horizon, and the LightGBM backend) are unchanged from
Section~\ref{sec:training_pipeline} so that the multi-fold result is directly
comparable to the single-split numbers reported earlier.
