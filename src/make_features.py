...
def build_validation_sets(
    ...
    for region_id, g in _iter_regions(train_df):
        ...
        hist = _score_history_for_window(scores, val_cutoff, include_score_features)
        for h in HORIZONS:
            target_idx = val_cutoff + 7 * h
            if target_idx < len(scores) and not np.isnan(scores[target_idx]):
                row = compute_window_features(values, dates, val_cutoff, hist, include_score_features)
                val_rows[h].append(row)
        ...
        for target_idx in cutoff_candidates:
            for h in HORIZONS:
                ...
                row = compute_window_features(values, dates, cutoff, hist, include_score_features)
                train_rows[h].append(row)
        ...
        for target_idx in label_idx:
            for h in HORIZONS:
                ...
                if cutoff < WINDOW_DAYS:
                    continue
                hist = _score_history_for_window(scores, cutoff, include_score_features)
                rows[h].append(compute_window_features(values, dates, cutoff, hist, include_score_features))
    ...
    for region_id, g in _iter_regions(train_df):
        ...
        for target_idx in label_idx:
            ...
            if cutoff < WINDOW_DAYS:
                continue
            hist = _score_history_for_window(scores, cutoff, include_score_features)
            rows.append(compute_window_features(values, dates, cutoff, hist, include_score_features))
...
def compute_window_features(
    ...
    for size in ROLLING_WINDOWS:
        ...
        q10 = np.nanpercentile(sub, 10, axis=0)
        q90 = np.nanpercentile(sub, 90, axis=0)
        ...
        prec = sub[:, WEATHER_COLS.index("prec")]
    ...
    prec91 = window[:, WEATHER_COLS.index("prec")]
    dry = prec91 == 0
    ...
    idx = {c: i for i, c in enumerate(WEATHER_COLS)}
    tmp_max7 = np.nanmean(window[-7:, idx["tmp_max"]])
    tmp_max28 = np.nanmean(window[-28:, idx["tmp_max"]])
    prec7 = np.nansum(window[-7:, idx["prec"]])
    prec28 = np.nansum(window[-28:, idx["prec"]])
    humidity7 = np.nanmean(window[-7:, idx["humidity"]])
    wind7 = np.nanmean(window[-7:, idx["wind"]])
    windmax7 = np.nanmean(window[-7:, idx["wind_max"]])
    surf_tmp7 = np.nanmean(window[-7:, idx["surf_tmp"]])
